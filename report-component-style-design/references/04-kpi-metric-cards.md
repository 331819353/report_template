...
- KPI overview cards may use scan-left value anchors inside each declared metric cell because the card carries several metrics in one horizontal reading row. The lead metric still owns the strongest value anchor, and every sibling metric must declare its own value baseline, unit placement, comparison baseline, and progress/target footer baseline.
- KPI judgment cards may place the visual anchor in a semantic hero zone, such as status icon, ring, threshold bullet, dimension breakdown, rating distribution, or semi-gauge. The hero zone must be backed by status dictionary, score range, target, threshold bands, or rating distribution data; it is not a decorative illustration.
- KPI goal execution cards may place the visual anchor in an execution hero zone, such as attainment ring, semi-gauge, linear target track, target-vs-actual pane, dot strip, stepper, timeline, or cumulative line. The hero zone must be backed by actual/target, gap, planned-vs-actual progress, milestone, deadline, or remaining-work fields; it is not a decorative progress graphic.
- KPI time-series analysis cards may place the visual anchor in a temporal evidence zone, such as a line/area body, comparison bar strip, cycle ring/period strip, volatility line, or forecast interval. The evidence zone must be backed by ordered series, baseline/cycle/volatility/forecast fields, grain, latest period, tooltip payload, and footer evidence; it is not a decorative sparkline.
- KPI comparison analysis cards may place the visual anchor in a comparison evidence zone, such as side-by-side values, grouped bars, market-share donut, radar profile, benchmark ruler, variance gauge, NPS scale, comparison table, or map-table pair. The evidence zone must be backed by comparable subject roles, shared metric definition, unit, period/grain, filter scope, benchmark or variance fields when needed, deterministic sort, tooltip payload, and footer evidence; it is not decorative competitive styling.
...
- KPI overview cards use the block/card title as the domain topic, such as `销售总览`, and may show body metric labels such as `销售额`, `同比增长`, or `目标完成率` because those labels distinguish different cells. They still must not repeat the domain title as a body label.
