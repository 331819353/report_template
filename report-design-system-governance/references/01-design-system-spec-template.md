...
| --- | --- | --- | --- | --- | --- | --- |
| KPI card | `W`, `H`, `P`, `CW`, `CH`, `centerX`; source from `$report-component-style-design` `12-internal-placement-algorithms.md` | `value + unit` group centered in the card body or declared landscape primary zone | Title top-left, definition/control top-right, comparison/target centered or primary-zone aligned, sparkline/ring/progress/icon in right-zone, metadata/evidence band bottom | small/standard/enhanced/wide/landscape | description -> summary -> mini evidence -> second comparison -> target progress | loading/empty/error/no-permission/stale preserve slot geometry |
