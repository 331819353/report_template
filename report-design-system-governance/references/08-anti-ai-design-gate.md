...
| `AI-RELATION-ORNAMENT` | Relationship visuals are ornamental. | Network nodes, ribbons, matrices, bubbles, and colored lines look sophisticated but lack relation task, method, source/target fields, strength, density limits, or exact evidence. | Use `relationshipAnalysisCardPattern`, declare relation task and evidence binding, keep the real `visualType`, and expose method, tooltip/detail, density fallback, and exact-value audit. |
