...
- Modern SaaS / BI Dashboard / UI Kit style evidence is required when that language is requested or claimed: screenshot/crop and DOM/CSS evidence for light page canvas, white-card surfaces, thin borders, small radii, restrained shadows, shared token family, first-viewport hierarchy, no nested-card/component pileup, and lightweight charts.
...
- Modern BI style-contract failure: requested or claimed modern SaaS / BI Dashboard / UI Kit styling lacks token/source proof, becomes an interchangeable white-card shell, piles unrelated components, makes charts visually overweight, or hides reading priority. Use `VIS-MODERN-BI-BASELINE-MISSING`, `VIS-GENERIC-SAAS-SHELL`, `VIS-COMPONENT-PILEUP`, `VIS-CHART-OVERWEIGHT`, or `VIS-HIERARCHY-FLAT`.
